...
class upload_helper {
    ...
    const STATUS_WAITING_FOR_WORKFLOW_PRE_ATTACHMENTS = 26;
    const STATUS_UPLOADING_ATTACHMENTS = 27;
    ...
    public static function get_status_string($statuscode) {
        ...
        switch ($statuscode) {
            ...
            case self::STATUS_WAITING_FOR_WORKFLOW_PRE_ATTACHMENTS :
                return get_string('mstatewaitingforworkflowpreattachments', 'block_opencast');
    ...
    protected function process_upload_job($job) {
        ...
        switch ($job->status) {
            ...
            case self::STATUS_CREATING_EVENT:
                ...
                if (!isset($event->identifier)) {
                    ...
                    if (empty($job->attachments)) {
                        ...
                        $this->update_status($job, self::STATUS_WAITING_FOR_WORKFLOW_PRE_ATTACHMENTS);
                    }
                    $stepsuccessful = true;
            ...
            case self::STATUS_WAITING_FOR_WORKFLOW_PRE_ATTACHMENTS:
                $isworkflowactive = $this->apibridge->is_workflow_running($job->opencasteventid);
                $stepsuccessful = !$isworkflowactive;
                if ($isworkflowactive) {
                    mtrace('... other workflow is still running, cannot upload attachments yet');
                } else {
            ...
            case self::STATUS_UPLOADING_ATTACHMENTS:
                $this->apibridge->media_package_upload_attachments($job);
    ...
    public static function get_opencast_attachments($uploadjobid) {
        ...
        $sql = "SELECT a.id, a.fileid, af.name, af.flavor
